...
- **名称查找与重载决议**：删除 `namespace` 后 ADL 的替代规则、用户如何为自己的
  类型重载运算符并被查找到。
- **`comp` 求值的阶段划分**：编译期求值、类型生成与实例化的精确顺序与收敛条件。
- **表达式级反射**：反射目前只规定类型、字段、方法与捕获；`^^` 作用于表达式的
  能力边界未定（影响 `check`）。
- **符号 mangling 与 ABI**：`comp` 生成类型的名称编码，决定 `.ncc.meta` 跨模块
  实例化去重。
- **内存模型**：`Atomic` 与 `MemoryOrder` 已有 API，但 happens-before 语义未规定。
