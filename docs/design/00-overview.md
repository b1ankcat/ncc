...
- **对象构造统一使用聚合初始化和构造函数**（聚合初始化是 C++20 已有语法，
  ...
  例外。
- **统一反射句柄**：`TypeInfo` 和 `ExprInfo` 分别表示编译期类型反射和表达式反射，
  同时也作为运行时类型描述符。`comp` 在编译期生成运行时描述符、字段访问器和方法桥接函数；
  运行时通过 `dynamic_type_of` 获取 `TypeInfo`，查询类型、字段和方法，并通过统一的
  `get_field`/`set_field`/`invoke` 访问。运行时反射句柄不能用于 `splice` 或生成新类型。
