...
- **[01-modules.md](01-modules.md)** - 模块系统
  - 删除头文件和 namespace，模块定义与使用
  - 核心库全局符号总表
  - `::` 的三种用途与内置模块
  - **名称查找**：关联模块替代 ADL，非成员运算符与泛型依赖调用的解析
...
本索引只重复总纲中已经批准的规则。尚未定稿的部分（名称查找与重载决议、`comp`
求值的阶段划分、表达式级反射、符号 mangling、内存模型）见
[00-overview.md](00-overview.md#文档结构)，相关位置标有“待确认”，不能视为新增
语言语法。
...
// 泛型类型（示意调用形式；核心库 Vector 的真实定义见 03-memory 与 04-reflection）
...
// 无歧义时可直接使用；同名导出用 import as 建立模块前缀区分
Client client;
parse("{...}");
```
...
00-overview 定义顶层规则，其余各章在其约束下展开，读它之前不必读别的。之后
01-modules → 02-types → 03-memory 建立基础，05-comp 与 04-reflection 是元编程
的核心（两者互为前提，建议对照阅读），15-examples 把前面的机制串起来。
工具链各章（10 至 14）可按需查阅，不必顺序读完。

与 C++ / Rust / Zig 的取舍对比见 [16-comparison.md](16-comparison.md)。
