...
| 语法 | 含义 | 参数处理 |
|------|------|----------|
| `X<A, B>` | 编译期调用 `X(^^A, ^^B)` | 自动加 `^^` 提升为 `TypeInfo` |
| `X(a, b)` | 普通函数调用 | 参数原样传递 |
...
| 用途 | 具体类型实现 | 泛型类定义 |
| 类型参数 | 无 | 通过 `<type T>` 声明 |
| 实例化 | 直接使用 | `CustomPtr<User>` 触发生成 |
| 成员函数 | 直接定义 | 直接定义（可使用类型参数） |
| 适用场景 | 非泛型类 | 需要类型参数的泛型类 |

`comp class` 适合直接书写成员实现的复杂泛型类；简单的聚合类型生成可以使用
`comp` 函数配合 `define_aggregate` 或 `define_class` API。两种写法都会生成
普通类定义，没有不同的生命周期或优化语义；需要资源管理时都必须明确定义
构造、析构、拷贝和移动操作。
