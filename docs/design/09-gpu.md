...
// GPU 数据并行
parallel<Device::Gpu>(gpu_data.size(), [gpu_data](size_t i) {
    gpu_data[i] = gpu_data[i] * 2.0f + 1.0f;
});
...
```cpp
// parallel 的 comp 函数实现（简化）
comp void parallel<Device device>(size_t n, auto func) {
    if (device == Device::Gpu) {
        // 编译期检查捕获列表
        for (auto capture : captures_of(^^decltype(func))) {
            auto capture_type = type_of(capture);
            
            if (!is_gpu_accessible(capture_type)) {
                static_assert(false, 
                    "GPU lambda 捕获了 CPU 内存类型 '{}'。"
                    "提示：使用 .to_device() 将数据迁移到 GPU。",
...
// is_gpu_accessible 的实现
comp bool is_gpu_accessible(type T) {
    // 1. 标量类型可以值捕获（复制到常量内存）
    if (is_scalar(T)) {
        return true;
    }
    ...
    // 2. 检查是否是 GPU 内存类型
    if (is_template_instantiation_of(T, ^^Vector)) {
        auto device_param = template_argument_at(T, 1);
        return device_param == Device::Gpu;
    }
...
1. **标量类型可以值捕获**（复制到常量内存）
2. **`Vector<T, Device::Gpu>` 等 GPU 类型可以捕获**
3. **`Vector<T, Device::Cpu>` 等 CPU 类型会报错**
...
// ✓ 正确：标量值捕获
int32_t scale = 2;
parallel<Device::Gpu>(gpu_data.size(), [gpu_data, scale](size_t i) {
    gpu_data[i] = gpu_data[i] * scale;  // scale 复制到常量内存
});
```
...
parallel<Device::Gpu>(data.size(), [data](size_t i) {
    data[i] = compute(i);
});
// 运行时自动在两个 GPU 上并行执行
```
...
1. **编译期检测**：
   - 编译器在编译期检测系统是否有 GPU
   - 如果代码中使用了 `Device::Gpu` 但系统无 GPU，编译错误：
   ```
...
void matmul_gpu(Matrix& c, const Matrix& a, const Matrix& b) {
    ...
    // GPU 计算
    parallel<Device::Gpu>(c_gpu.rows(), c_gpu.cols(), [=](size_t i, size_t j) {
        float sum = 0.0f;
